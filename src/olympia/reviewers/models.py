...
class ReviewerScore(ModelBase):
    ...
    def get_total(cls, user):
        ...
        val = (
            ReviewerScore.objects.no_cache()
            .filter(user=user)
...
class AutoApprovalSummary(ModelBase):
    ...
    def calculate_weight(self):
        ...
        factors = {
            ...
            'abuse_reports': min(
                ...
                .filter(created__gte=one_year_ago)
                .count()
                * 10,
                100,
            ),
        ...
        try:
            factors = {
                ...
                'uses_eval_or_document_write': (
                    20
                    if self.check_uses_eval_or_document_write(self.version)
                    else 0
                ),
                ...
                'uses_native_messaging': (
                    20 if self.check_uses_native_messaging(self.version) else 0
                ),
                # remote scripts: 40.
                'uses_remote_scripts': (
                ...
                'violates_mozilla_conditions': (
                    20
                    if self.check_violates_mozilla_conditions(self.version)
                    else 0
                ),
                # libraries of unreadable code: 10.
                'uses_unknown_minified_code': (
                    10
                    if self.check_uses_unknown_minified_code(self.version)
                    else 0
                ),
            ...
            factors = {'no_validation_result': 200}
        return factors
    ...
    def check_for_linter_flag(cls, version, flag):
        ...
        return any(
            _check_for_linter_flag_in_file(file_)
            for file_ in version.all_files
        )
    ...
    def check_for_metadata_property(cls, version, prop):
        def _check_for_property_in_linter_metadata_in_file(file_):
            try:
                ...
                raise AutoApprovalNoValidationResultError()
            validation_data = json.loads(validation.validation)
            return validation_data.get('metadata', {}).get(prop)

        return any(
            _check_for_property_in_linter_metadata_in_file(file_)
            for file_ in version.all_files
        )
    ...
    def check_uses_eval_or_document_write(cls, version):
        return cls.check_for_linter_flag(
            version, 'NO_DOCUMENT_WRITE'
        ) or cls.check_for_linter_flag(version, 'DANGEROUS_EVAL')
